...
def tmdb_discover_by_genres(genre_ids_tuple, exclude_id=None):
    """Bassin de candidats large et diversifié : trois requêtes distinctes sur TMDB pour ne pas
    se retrouver qu'avec des sorties récentes populaires.
    1. Popularité décroissante toutes époques (3 pages).
    2. Mieux notés avec minimum de votes — ramène les classiques (2 pages).
    3. Genres exacts AND — films qui partagent TOUS les genres cibles (1 page).
    La déduplication + scoring TF-IDF font ensuite le tri."""
    if not genre_ids_tuple:
        return []
    genre_str_or  = "|".join(str(g) for g in genre_ids_tuple)
    genre_str_and = ",".join(str(g) for g in genre_ids_tuple)
    queries = (
        [{"sort_by": "popularity.desc",    "with_genres": genre_str_or,  "page": p} for p in (1, 2, 3)]
        + [{"sort_by": "vote_average.desc", "with_genres": genre_str_or,
            "vote_count.gte": "200",        "page": p} for p in (1, 2)]
        + [{"sort_by": "vote_average.desc", "with_genres": genre_str_and,
            "vote_count.gte": "100",        "page": 1}]
    )
    results = []
    for params in queries:
        data = _tmdb_get("/discover/movie", {"language": "fr-FR", **params})
        if data:
            results.extend(data.get("results", []))
    seen, uniq = set(), []
    for m in results:
        if m["id"] != exclude_id and m["id"] not in seen and m.get("vote_count", 0) >= 10:
            uniq.append(m)
            seen.add(m["id"])
    return uniq
...
def rank_by_content_similarity(query_genre_names, candidates, genre_map, top_n=10):
    """Score hybride à deux composantes :
    - 70 % : similarité cosinus TF-IDF sur les genres (notre algorithme content-based).
    - 30 % : note TMDB normalisée pondérée par le nombre de votes (bonus qualité, façon
      Bayesian average — même formule que le classement IMDb, voir notebook section 4).
    Résultat : les films avec genres très proches ET bien notés remontent, les films récents
    populaires mais mal notés ou hors-sujet redescendent."""
    if not candidates:
        return []
    ...
    # Score qualité : note moyenne pondérée par le nombre de votes (évite les films
    # notés 10/10 par 3 personnes). On normalise entre 0 et 1 (note max = 10 sur TMDB).
    VOTE_MIN = 50  # seuil minimum pour que la note compte vraiment
    C = 6.5        # note "neutre" par défaut si pas assez de votes
    quality = []
    for c in candidates:
        v = c.get("vote_count", 0)
        r = c.get("vote_average", C)
        q = (v / (v + VOTE_MIN)) * r + (VOTE_MIN / (v + VOTE_MIN)) * C
        quality.append(q / 10.0)  # normalisation 0-1

    final_scores = [0.70 * s + 0.30 * q for s, q in zip(sims, quality)]
    scored = sorted(zip(candidates, final_scores), key=lambda x: x[1], reverse=True)
    return scored[:top_n]
...
def render_movie_detail(movie, genre_map):
    # Ancre de scroll — le JS injecté plus bas va scroller jusqu'ici automatiquement
...
def page_decouvrir():
    ...
    if sel:
        ...
        # Scroll automatique vers la fiche du film avec animation fluide,
        # déclenché 200 ms après le rendu pour laisser le DOM se stabiliser
        st.components.v1.html(
            """
            <script>
            setTimeout(function() {
                var el = window.parent.document.getElementById('detail-anchor');
